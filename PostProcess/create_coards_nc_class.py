#-------------------------------------------------------------------------------
# Name:        create_coards_nc_class.py
...
__prog__ = 'create_coards_nc_class.py'
__version__ = '0.0.0'
__author__ = 's03mm5'
...
def find_metric_in_flist_names(file_list, metric):
    '''
...
def create_coards_nc_dset(form, metric_obj):
    """
    form, nc_metrics, metric_obj
    return list of NC files names
    """
    func_name =  __prog__ + ' create_coards_nc_dset'
...
class Coard_nc_defn(object, ):
    '''
    instantiate
    '''
    def __init__(self, metric, fname, land_use, out_dir, study, delete_flag = True):
        """
        ...
        long_name = 'unknown'
        if metric == 'npp':
            long_name = 'flux co2 from dry matter (net primary production) npp'
        elif metric in FLUXES:
            ...
            if isfile(fout_name):
                if delete_flag:
                    try:
                        ...
                        print('File: {} already exists but could not delete'.format(fout_name))
                        fout_name = None
                else:
